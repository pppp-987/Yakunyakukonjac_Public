...
class AwsConfigureWin(BaseWin):
        ...
        while not self.window.metadata["is_exit"]:
            ...
            # AWS設定を行うスレッドが終了したなら
                ...
                self.aws_configure_thread_end_event(
                    is_successful=values["-aws_configure_thread_end-"],  # AWSの設定の変更中に中断されてないかどうか
                    check_access_aws_thread=check_access_aws_thread,  # AWS接続テストを行うスレッド
                )
    ...
    def aws_configure_thread_end_event(self, is_successful, check_access_aws_thread):
        """AWS設定スレッドが終了した時の処理
